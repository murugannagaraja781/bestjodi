...
if(isset($_SESSION['franchies_user_id']))
{
if($a!='' || $b!='' || $c!='' || $d!='' || $e!='' || $f!='' || $g!='' || $h!='' && $_SESSION['franchies_user_id']!='')
{
$i="and franchies_id='".$_SESSION['franchies_user_id']."'";
}
elseif($a=='' && $b=='' && $c=='' && $d=='' && $e=='' && $f=='' && $g=='' && $h=='' && $_SESSION['franchies_user_id']!='')
{
$i="where franchies_id='".$_SESSION['franchies_user_id']."'";
}
}
else
{
...
$rows = mysqli_num_rows($DatabaseCo->dbLink->query("select * from register_view r,payments p where r.matri_id=p.pmatri_id   and p.exp_date<'$today' $b $c $d $e $f $g $h $i"));
$sql =  "select * from register_view r,payments p where r.matri_id=p.pmatri_id and p.exp_date<'$today' $b $c $d $e $f $g $h $i ORDER BY reg_date DESC LIMIT $start, $limit";
}
else
{
$rows = mysqli_num_rows($DatabaseCo->dbLink->query("select * from register_view $a $b $c $d $e $f $g $h $i"));
$sql = "SELECT * FROM register_view $a $b $c $d $e $f $g $h $i and franchies_id='".$_SESSION['franchies_user_id']."' ORDER BY reg_date DESC LIMIT $start, $limit";  
}
...
<div class="col-xs-12">
  <div class="nodata-avail">
    <h5>Search : 
      <?php
if(isset($_SESSION['search_gender'])){ echo "<b>Gender</b> : ".$_SESSION['search_gender'].",  ";}?>
      <?php if(isset($_SESSION['search_keyword'])){ echo "<b>Keyword</b> : ".$_SESSION['search_keyword'].", ";}?> 			
      <?php if(isset($_SESSION['search_castenm'])){ echo "<b>Caste</b> : ".$_SESSION['search_castenm'].", ";}?>
      <?php if(isset($_SESSION['search_relnm'])){ echo "<b>Religion</b> : ".$_SESSION['search_relnm'].", ";}?> 			 			 
      <?php if(isset($_SESSION['search_cntnm'])){ echo "<b>Country</b> : ".$_SESSION['search_cntnm'].", ";}?>
      <?php if(isset($_SESSION['search_statenm'])){ echo "<b>State</b> : ".$_SESSION['search_statenm'].", ";}?> 							 			 
      <?php if(isset($_SESSION['search_citynm'])){ echo "<b>City</b> : ".$_SESSION['search_citynm'].", ";}?>
    </h5>
  </div>
</div>
<?php 
